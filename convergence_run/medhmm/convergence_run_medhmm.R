...
# Define the random data sets to be inspected with convergence
set.seed(123)
sampled_datasets<-base::sample(1:128,max(s))
#========= Load datasets and extract the chosen ones====================================================
#---------Two state scenario chose appropriate number of datasets---------------------------------------

#In order to run the scripts without any issues using smellius we need to adjust the below data directory
two_state<-readRDS(paste0("/gpfs/home1/adacko/data/two_state_design_2dep_final.rds"))
two_state_chosen_datasets<-list()
for(obs in o){
  dwell_list<-list()
  for(dur in d){
    sampled_data<-list()
    for(sampl in sampled_datasets){
      sampled_data[[which(sampled_datasets==sampl)]]<-two_state[[obs]][[dur]][[sampl]]
    }
    dwell_list[[dur]]<-sampled_data
  }
  two_state_chosen_datasets[[obs]]<-dwell_list
}
rm(two_state)
...
# Save data files
#In order to run the scripts without any issues using smellius we need to adjust the below directory  to the location of data portal
saveRDS(list(two_state_chosen_datasets,sampled_datasets), paste0("./data/convergence_medhmm_run_chosen_data_two_states.rds"))
saveRDS(list(three_state_chosen_datasets,sampled_datasets), paste0("./data/convergence_medhmm_run_chosen_data_three_states.rds"))
saveRDS(list(four_state_chosen_datasets,sampled_datasets), paste0("./data/convergence_medhmm_run_chosen_data_four_states.rds"))
  ...
  if(m==2){
    ...
    if(m==3){
      ...
      if(d==1){
        ...
        #-Dwell time starting values
        dwell_mu_var<-log(matrix(
          data = c(exp_mean4, exp_sigma4*1.5,
                   exp_mean4, exp_sigma4*1.5,
                   exp_mean4, exp_sigma4*1.5),
          byrow = T,
          ncol = 2,
          nrow = m))
...
#In order to run the scripts without any issues using smellius we need to adjust the below data directory
saveRDS(sampled_datasets,paste0("./convergence_run/medhmm/outputs/randomly_chosen_datasets_medhmmm.rds"))
...
#This function produces pdfs with trace plots that
#In order to run the scripts without any issues using smellius we need to adjust the below data directory
