F#-------------------------------------------------------------------------------
# Convergence checks script for medhmm extra
#-------------------------------------------------------------------------------
...
set.seed(123)
sampled_datasets<-base::sample(1:128,max(s))
...
# Save data files
#In order to run the scripts without any issues using smellius we need to adjust the below data directory
saveRDS(list(three_state_chosen_datasets2,sampled_datasets), paste0("./data/convergence_medhmm_run_chosen_data_three_mixed_states.rds"))

#In order to run the scripts without any issues using smellius we need to adjust the below data directory
two_state_chosen_datasets<-readRDS("./data/convergence_medhmm_run_chosen_data_two_states.rds")
two_state_chosen_datasets<-two_state_chosen_datasets[[1]]
three_state_chosen_datasets<-readRDS("./data/convergence_medhmm_run_chosen_data_three_states.rds")
three_state_chosen_datasets<-three_state_chosen_datasets[[1]]
four_state_chosen_datasets<-readRDS("/./data/convergence_medhmm_run_chosen_data_four_states.rds")
four_state_chosen_datasets<-four_state_chosen_datasets[[1]]
three_state_chosen_datasets2<-readRDS("./data/convergence_medhmm_run_chosen_data_three_mixed_states.rds")
#==================================================================================================================================================================================================
#Outstart clean
#==================================================================================================================================================================================================
...
out_list1 <-pbmcapply::pbmclapply(1:nrow(pars), function(parameter_row) {
  ...
  if(m==2){
    ...
    emiss_distr <- matrix(c( 30,60*1.1,
                             60, 120*1.1), nrow = m, byrow = TRUE)
    emiss_distr2 <- matrix(c(60,(120+0.25*120)*1.1,
                             30, (60+0.25*60)*1.1), nrow = m, byrow = TRUE)
    #---Emission distribution starting values for second chain
    emiss_distr_2nd_chain <- matrix(c( 30*(base::sample(c(0.9,1.1),1)),60*1.1*(base::sample(c(0.9,1.1),1)),
                                       60*(base::sample(c(0.9,1.1),1)), 120*1.1*(base::sample(c(0.9,1.1),1))), nrow = m, byrow = TRUE)
    emiss_distr2_2nd_chain <- matrix(c(60*(base::sample(c(0.9,1.1),1)),(120+0.25*120)*1.1*(base::sample(c(0.9,1.1),1)),
                                       30*(base::sample(c(0.9,1.1),1)), (60+0.25*60)*1.1*(base::sample(c(0.9,1.1),1))), nrow = m, byrow = TRUE)
    ...
    if(d==1){
      ...
      dwell_mu_var<-log(matrix(
        data = c(exp_mean1,exp_sigma1*1.1,
                 exp_mean1,exp_sigma1*1.1),
        byrow = T,
        ncol = 2,
        nrow = m))

      dwell_mu_var_2nd_chain<-matrix(
        data = c(log(exp_mean1),log(exp_sigma1)*1.05*(base::sample(c(0.95,1.05),1)),
                 log(exp_mean1),log(exp_sigma1)*1.05*(base::sample(c(0.95,1.05),1))),
        byrow = T,
        ncol = 2,
        nrow = m)
      ...
      #-Dwell time starting values
      dwell_mu_var<-log(matrix(
        data = c(exp_mean2, exp_sigma2*1.1,
                 exp_mean2,  exp_sigma2*1.1),
        byrow = T,
        ncol = 2,
        nrow = m))

      dwell_mu_var_2nd_chain<-matrix(data = c(log(exp_mean2), log(exp_sigma2)*1.05*base::sample(c(0.95,1.05),1),
                                              log(exp_mean2),  log(exp_sigma2)*1.05*base::sample(c(0.95,1.05),1)), byrow = T,ncol = 2,nrow = m)
      #-Dwell time hyper priors
      ...
      #-Dwell time starting values
      dwell_mu_var<- log(matrix(
        data = c(exp_mean3,exp_sigma3*1.1,
                 exp_mean3,exp_sigma3*1.1),
        byrow = T,
        ncol = 2,
        nrow = m))
      dwell_mu_var_2nd_chain<- matrix(
        data = c(log(exp_mean3),log(exp_sigma3)*1.05*(base::sample(c(0.95,1.05),1)),
                 log(exp_mean3),log(exp_sigma3)*1.05*(base::sample(c(0.95,1.05),1))),
        byrow = T,
        ncol = 2,
        nrow = m)
      #-Dwell time hyper priors
      ...
      #-Dwell time starting values
      dwell_mu_var<- log(matrix(
        data = c(exp_mean4,exp_sigma4*1.1,
                 exp_mean4,exp_sigma4*1.1),
        byrow = T,
        ncol = 2,
        nrow = m))
      dwell_mu_var_2nd_chain<- matrix(
        data = c(log(exp_mean4),log(exp_sigma4)*1.05*(base::sample(c(0.95,1.05),1)),
                 log(exp_mean4),log(exp_sigma4)*1.05*(base::sample(c(0.95,1.05),1))),
        byrow = T,
        ncol = 2,
        nrow = m)
      #-Dwell time hyper priors
    ...
    if(m==3){
      ...
      #---Emission distribution starting values
      emiss_distr <- matrix(c(10,20*1.1,
                              30, 60*1.1,
                              60,120*1.1), nrow = m, byrow = TRUE)
      emiss_distr2 <- matrix(c(60,(120+0.25*120)*1.1,
                               30, (60+0.25*60)*1.1,
                               10,(20+20*0.25)*1.1), nrow = m, byrow = TRUE)
      #---Emission distribution starting values 2nd chain
      emiss_distr_2nd_chain <- matrix(c(10*base::sample(c(0.9,1.1),1),20*1.1*base::sample(c(0.9,1.1),1),
                                        30*base::sample(c(0.9,1.1),1), 60*1.1*base::sample(c(0.9,1.1),1),
                                        60*base::sample(c(0.9,1.1),1),120*1.1*base::sample(c(0.9,1.1),1)), nrow = m, byrow = TRUE)
      emiss_distr2_2nd_chain<- matrix(c(60*base::sample(c(0.9,1.1),1),(120+0.25*120)*1.1*base::sample(c(0.9,1.1),1),
                                        30*base::sample(c(0.9,1.1),1), (60+0.25*60)*1.1*base::sample(c(0.9,1.1),1),
                                        10*base::sample(c(0.9,1.1),1),(20+20*0.25)*1.1*base::sample(c(0.9,1.1),1)), nrow = m, byrow = TRUE)

      #---Gamma starting values
      gamma   <- matrix(c(0, 0.5,0.5,
                          0.5, 0, 0.5,
                          0.5, 0.5, 0), ncol = m, byrow = TRUE)
      gamma2   <- matrix(c(0, 0.5,0.5,
                           0.5, 0, 0.5,
                           0.5, 0.5, 0), ncol = m, byrow = TRUE)
      diag(gamma2)=NA
      gamma2<-t(matrix(t(gamma2)[which(!is.na(gamma2))],nrow=nrow(gamma2)-1,ncol=ncol(gamma2)))+matrix(rnorm(m*(m-1),mean=0,sd=0.04),ncol=m-1) %>% abs()
      ...
      if(d==1){
        ...
        dwell_mu_var_2nd_chain<-matrix(data = c(log(exp_mean1), log(exp_sigma1)*1.05*base::sample(c(0.95,1.05),1),
                                                log(exp_mean1), log(exp_sigma1)*1.05*base::sample(c(0.95,1.05),1),
                                                log(exp_mean1), log(exp_sigma1)*1.05*base::sample(c(0.95,1.05),1)),
                                       byrow = T,
                                       ncol = 2,
                                       nrow = m)
        #-Dwell time hyper priors
        dwell_hyp_prior<-list(d_mu0=log(c(exp_mean1,exp_mean1,exp_mean1)),
                              s2_0=log(rep(10, m)),
                              alpha.sigma20	= rep(0.01, m),
                              beta.sigma20	= rep(0.01, m),
                              alpha.tau20		= rep(0.01, m),
        ...
        dwell_mu_var_2nd_chain<-matrix(data = c(log(exp_mean2), log(exp_sigma2)*1.05*base::sample(c(0.95,1.05),1),
                                                log(exp_mean2), log(exp_sigma2)*1.05*base::sample(c(0.95,1.05),1),
                                                log(exp_mean2), log(exp_sigma2)*1.05*base::sample(c(0.95,1.05),1)),
                                       byrow = T,
                                       ncol = 2,
                                       nrow = m)
        #-Dwell time hyper priors
        dwell_hyp_prior<-list(d_mu0=log(c(exp_mean2,exp_mean2,exp_mean2)),
                              s2_0=log(rep(10, m)),
                              alpha.sigma20	= rep(0.01, m),
                              beta.sigma20	= rep(0.01, m),
                              alpha.tau20		= rep(0.01, m),
        ...
        dwell_mu_var_2nd_chain<-matrix(data = c(log(exp_mean3), log(exp_sigma3)*1.05*base::sample(c(0.95,1.05),1),
                                                log(exp_mean3), log(exp_sigma3) *1.05*base::sample(c(0.95,1.05),1),
                                                log(exp_mean3), log(exp_sigma3)*1.05*base::sample(c(0.95,1.05),1)),
                                       byrow = T,
                                       ncol = 2,
                                       nrow = m)
        #-Dwell time hyper priors
        dwell_hyp_prior<-list(d_mu0=log(c(exp_mean3,exp_mean3,exp_mean3)),
                              s2_0=log(rep(10, m)),
                              alpha.sigma20	= rep(0.01, m),
                              beta.sigma20	= rep(0.01, m),
                              alpha.tau20		= rep(0.01, m),
        ...
        dwell_mu_var_2nd_chain<-matrix(data = c(log(exp_mean4), log(exp_sigma4)*1.05*base::sample(c(0.95,1.05),1),
                                                log(exp_mean4), log(exp_sigma4) *1.05*base::sample(c(0.95,1.05),1),
                                                log(exp_mean4), log(exp_sigma4)*1.05*base::sample(c(0.95,1.05),1)),
                                       byrow = T,
                                       ncol = 2,
                                       nrow = m)
        #-Dwell time hyper priors
        dwell_hyp_prior<-list(d_mu0=log(c(exp_mean4,exp_mean4,exp_mean4)),
                              s2_0=log(rep(10, m)),
                              alpha.sigma20	= rep(0.01, m),
                              beta.sigma20	= rep(0.01, m),
                              alpha.tau20		= rep(0.01, m),
      ...
      sim_data<-three_state_chosen_datasets2
        ...
        dwell_mu_var_2nd_chain<-matrix(data = c(log(exp_mean2), log(exp_sigma2)*1.05*base::sample(c(0.95,1.05),1),
                                       ...
                                       nrow = m)
        #-Dwell time hyper priors
        dwell_hyp_prior<-list(d_mu0=log(c(exp_mean2,exp_mean3,exp_mean4)),
                              s2_0=log(rep(10, m)),
                              alpha.sigma20	= rep(0.01, m),
                              beta.sigma20	= rep(0.01, m),
                              alpha.tau20		= rep(0.01, m),
      ...
      if(m==4){
        ...
        emiss_distr <- matrix(c(10,20*1.1,
                                30, 60*1.1,
                                60,120*1.1,
                                98,150*1.1), nrow = m, byrow = TRUE)
        emiss_distr2 <- matrix(c(98,(150+0.25*150)*1.1,
                                 60, (120+0.25*120)*1.1,
                                 30,(60+0.25*60)*1.1,
                                 10,(20+0.25*20)*1.1), nrow = m, byrow = TRUE)
        #---Emission distribution starting values 2nd chain
        emiss_distr_2nd_chain <- matrix(c(10*base::sample(c(0.9,1.1),1),20*1.1*base::sample(c(0.9,1.1),1),
                                          30*base::sample(c(0.9,1.1),1), 60*1.1*base::sample(c(0.9,1.1),1),
                                          60*base::sample(c(0.9,1.1),1),120*1.1*base::sample(c(0.9,1.1),1),
                                          98*base::sample(c(0.9,1.1),1),150*1.1*base::sample(c(0.9,1.1),1)), nrow = m, byrow = TRUE)
        emiss_distr2_2nd_chain <- matrix(c(98*base::sample(c(0.9,1.1),1),(150+0.25*150)*1.1*base::sample(c(0.9,1.1),1),
                                           60*base::sample(c(0.9,1.1),1), (120+0.25*120)*1.1*base::sample(c(0.9,1.1),1),
                                           30*base::sample(c(0.9,1.1),1),(60+0.25*60)*1.1*base::sample(c(0.9,1.1),1),
                                           10*base::sample(c(0.9,1.1),1),(20+0.25*20)*1.1*base::sample(c(0.9,1.1),1)), nrow = m, byrow = TRUE)
        #---Gamma starting values
        gamma   <- matrix(c(0, 1/3,1/3,1/3,
                            1/3,0,0.1/3,1/3,
                            1/3,1/3,0,1/3,
                            1/3,1/3,1/3,0), ncol = m, byrow = TRUE)

        gamma2   <- matrix(c(0, 1/3,1/3,1/3,
                             1/3,0,0.1/3,1/3,
                             1/3,1/3,0,1/3,
                             1/3,1/3,1/3,0), ncol = m, byrow = TRUE)
        ...
        if(d==1){
          ...
          dwell_mu_var_2nd_chain<-matrix(
            ...
            nrow = m)
          #-Dwell time hyper priors
          dwell_hyp_prior<-list(d_mu0=log(c(exp_mean1,exp_mean1,exp_mean1,exp_mean1)),
                                s2_0=log(rep(10, m)),
                                alpha.sigma20	= rep(0.01, m),
                                beta.sigma20	= rep(0.01, m),
                                alpha.tau20		= rep(0.01, m),
          ...
          dwell_mu_var_2nd_chain<-matrix(
            data = c(log(exp_mean2), log(exp_sigma2)*1.05*base::sample(c(0.95,1.05),1),
                     log(exp_mean2),  log(exp_sigma2)*1.05*base::sample(c(0.95,1.05),1),
                     log(exp_mean2),  log(exp_sigma2)*1.05*base::sample(c(0.95,1.05),1),
                     log(exp_mean2),  log(exp_sigma2)*1.05*base::sample(c(0.95,1.05),1)),
            byrow = T,
            ...
            nrow = m)
          #-Dwell time hyper priors
          dwell_hyp_prior<-list(d_mu0=log(c(exp_mean2,exp_mean2,exp_mean2,exp_mean2)),
                                s2_0=log(rep(10, m)),
                                alpha.sigma20	= rep(0.01, m),
                                beta.sigma20	= rep(0.01, m),
                                alpha.tau20		= rep(0.01, m),
          ...
          dwell_mu_var_2nd_chain<-matrix(data = c(log(exp_mean4), log(exp_sigma4)*1.05*base::sample(c(0.95,1.05),1),
                                         ...
                                         nrow = m)
          #-Dwell time hyper priors
          dwell_hyp_prior<-list(d_mu0=log(c(exp_mean4,exp_mean4,exp_mean4,exp_mean4)),
                                s2_0=log(rep(10, m)),
                                alpha.sigma20	= rep(0.01, m),
                                beta.sigma20	= rep(0.01, m),
                                alpha.tau20		= rep(0.01, m),
          ...
          dwell_mu_var_2nd_chain<-matrix(data = c(log(exp_mean3), log(exp_sigma3)*1.05*base::sample(c(0.95,1.05),1),
                                         ...
                                         nrow = m)
          #-Dwell time hyper priors
          dwell_hyp_prior<-list(d_mu0=log(c(exp_mean3,exp_mean3,exp_mean3,exp_mean3)),
                                s2_0=log(rep(10, m)),
                                alpha.sigma20	= rep(0.01, m),
                                beta.sigma20	= rep(0.01, m),
                                alpha.tau20		= rep(0.01, m),
  ...
  if(d==4 && o==3){
    out_list1<-try(list(chain1=out1,MAP=MAP1))
    #In order to run the scripts without any issues using snellius we need to adjust the below target directory
    try(saveRDS(out_list1, paste0("./convergence_run/medhmm/outputs/convergence_run_one_part",
                               "_m",m,
                               "_nt",o,
                               "_dwell",d,
                               "dt_set",s,
  ...
  # Save output
  #In order to run the scripts without any issues using snellius we need to adjust the below target directory
  try(saveRDS(case_out, paste0("./convergence_run/medhmm/outputs/convergence_run",
                               "_m",m,
                               "_nt",o,
                               "_dwell",d,
                               "dt_set",s,
...
saveRDS(sampled_datasets,paste0(".4567890/convergence_run/medhmm/outputs/randomly_chosen_datasets_medhmmm.rds"))


# Calling expand.grid() Function to create the parameter dataframe
#pars2 = expand.grid(m=2,o=1:3,d=d,s=1:5)
pars3 = expand.grid(m=3,o=1.3,d=1:4,s=1:5)
pars4 = expand.grid(m=4,o=1:3,d=1:4,s=1:5)
...
gelman_list_all3 <-pbmcapply::pbmclapply(1:nrow(pars3), function(parameter_row) {
  ...
  #In order to run the scripts without any issues using snellius we need to adjust the below target directory
  dataset<- readRDS(paste0("./convergence_run/medhmm/outputs/convergence_run",
                           "_m",m,
                           "_nt",o,
                           "_dwell",d,
                           "dt_set",s,
...
#In order to run the scripts without any issues using snellius we need to adjust the below target directory
saveRDS(gelman_list_all3,paste0("./convergence_run/medhmm/outputs/all_gelaman_medhmm_m3.rds"))
...
#In order to run the scripts without any issues using snellius we need to adjust the below target directory
saveRDS(gelman_list_all4,paste0("./convergence_run/medhmm/outputs/all_gelaman_medhmm_m4.rds"))
...
# Calling expand.grid() Function to create the parameter dataframe
pars = expand.grid(m=3:4,o=1:3,d=1:5,s=1:5)
...
pdf("./convergence_run/medhmm/outputs/trace_plots_medhmm_m3_m4.pdf")
for(parameter in 1:nrow(pars)){
  ...
  #In order to run the scripts without any issues using snellius we need to adjust the below data directory
  dataset<- try(readRDS(paste0("./convergence_run/medhmm/outputs/convergence_run",
                           "_m",m,
                           "_nt",o,
                           "_dwell",d,
                           "dt_set",s,
