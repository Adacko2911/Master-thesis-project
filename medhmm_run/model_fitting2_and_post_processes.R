#----------------------------------------------------------------------------------------------------------------------------------------------
# Fit model for M hidden states, 2 dependent variable, 80 subjects, o length of time series and d duration of a hidden state and save results
#----------------------------------------------------------------------------------------------------------------------------------------------
...
library(medHMM) #for medhmm
require(pbmcapply) #for paraller computing
require(tidyverse) #for data wrangling

#------------------------------------------------------------------------------
#function removing global variables if exist
...
#  will to be fitted to the medhmm model (s)
...
if(m==2){
  #simulated data
  sim_data<-readRDS(paste0("/gpfs/home1/adacko/data/two_state_design_2dep_final.rds"))
  ...
  if(m==3){
    #simulated data
    sim_data<-readRDS(paste0("/gpfs/home1/adacko/data/three_state_design_2dep_final.rds"))
    #---Emission distribution starting values
    ...
    if(m==4){
      #simulated data
      sim_data<-readRDS(paste0("/gpfs/home1/adacko/data/four_state_design_2dep_final.rds"))
      #---Emission distribution starting values
      emiss_distr <- matrix(c(10,20*1.2,
                              30, 60*1.2,
                              60,120*1.2,
...
out_list1 <-pbmcapply::pbmclapply(1:cores, function(s) {
 ...
 if(d==1){
  ...
  case_out<- try(medHMM_cont(s_data = input_data_mhmm$observations,
                             ...
                             show_progress = TRUE,
                             mcmc = list(J = J, burn_in = B), return_path = TRUE, max_dwell = max_dwell))
 print(.Last.value)
  options(show.error.messages = TRUE)
  case_out[["time"]] <- Sys.time() - ti

  # Calculate necessary statistics to be saved
  if(!is.null(case_out)){#vit_medhmm_cont to be changed
    state_decoding<-try(local_decoding(out=case_out))
    true_st<-as.data.frame(input_data_mhmm$states)
    state_decoding_table<-try(data.frame(subject=true_st[,1],true_state_decoding=true_st[,-1],vit_state_decoding=state_decoding$state))
    #rm(state_decoding)
    #---MAP estimates---------
    MAP<-MAP_medhmm(case_out =case_out,iteration=s,J=J,B=B,m=m)
    # Define new output object
    
    case_out <- list(decoding = state_decoding_table,MAP=MAP)
...
# Save output as a list of 128 repetitions of 1/36 scenarios:
saveRDS(out_list1, paste0("/gpfs/home1/adacko/medhmm_run/outputs/medhmm_list_output",
                          "_m",m,
                          "_nt",o,
                          "_dwell",d,
                          "_it",J,
...
# four state model takes approximately 24s to run for 15 iterations 4.8 min for 12 scenarios; 1hr 30min for each 4000 it
# three state model takes approximately 18s to run for 15 iterations 3.6 min for 12 scenarios; 1hr for each 4000 it
# two state model takes approximately 15s to run for 15 iterations 3 min for 12 scenarios; 1hr 10 min for each 4000 it

# assuming each repetition of the fitting model to the same scenario is performed on different node
# those times are on my computer modeling the 4 state model would take approximately 1hr 30 max max

namessd<-c()
namessd<-append(namessd,paste0("st_",m,"_obs_",o,"_dwell_",d,"_medhmm"))
dwell_time_names<-c(as.character(round(exp_mean1,2)),as.character(round(exp_mean2,2)),as.character(round(exp_mean3,2)),as.character(round(exp_mean4,2)))
...
if(m==2){
  ...
  true_emiss_var_bar<-c(60,120)
  true_emiss_mu_bar<-c(30,60)
  true_emiss_varmu_bar2 <- rep(9,m)
  true_emiss_var_bar2<-c(120+0.25*120, 60+0.25*60)
  true_emiss_mu_bar2<-c(60,30)
  if(d==1){
    true_dwell_mu_bar<-rep(log(exp_mean1),m)
    true_dwell_varmu_bar<-rep(0.2,m)
    true_dwell_var_bar<-rep(log(exp_sigma1),m)
    true_gamma_prob_bar<-as.vector(matrix(c(0, 1,
                                            1, 0), ncol = m, byrow = TRUE))
  }else if(d==2){
...
saveRDS(out_list2, paste0("/gpfs/home1/adacko/medhmm_run/outputs/post_processed/medhmm_post_proc_output",
                          "_m",m,
                          "_nt",o,
                          "_dwell",d,
                          "_it",J,
  ...
  post_proc_out<-rbind(emiss_mu_bar_out,emiss_mu_bar2_out,emiss_var_bar_out,emiss_var_bar2_out,emiss_varmu_bar_out,emiss_varmu_bar2_out,dwell_mu_bar_out,dwell_var_bar_out,dwell_varmu_bar_out) %>% round(.,1)
  post_proc_out$estimates <- rownames(post_proc_out)
  post_proc_out$model<-rep("medhmm",nrow(post_proc_out))
  post_proc_out$state<-rep(m,nrow(post_proc_out))
  post_proc_out$observations<-rep(observation_lenght[o],nrow(post_proc_out))
...
if(m==2 & o==1 & d==4){
  ...
  pars1 = expand.grid(d=d1,o=o1,m=m1)
  n_dep=2
  list_post_est<-list()
  list_decod_acc<-list()
  list_switches_est<-list()
  list_dwell_emp<-list()
  dwell_time_names<-c(1.4,3.5,19.5,99.5)
  obs_names<-c(200,500,1000)
  for(parameter in 1:nrow(pars1)){
    m  <- pars1[parameter,3]
    o  <- pars1[parameter,2]
    d  <- pars1[parameter,1]

    post_data<-readRDS(paste0("/gpfs/home1/adacko/medhmm_run/outputs/post_processed/medhmm_post_proc_output",
                              "_m",m,
                              "_nt",o,
                              "_dwell",d,
                              "_it",J,
    ...
    true_cols<-data.frame(mean_emp_dwell_true=post_data$empirical_dwell_time_true$mean_emp_dwell,median_emp_dwell_true=post_data$empirical_dwell_time_true$median_emp_dwell, ci_025_emp_dwell_true=post_data$empirical_dwell_time_true$ci_025,ci_975_emp_dwell_true=post_data$empirical_dwell_time_true$ci_975)
     ...
     emp_dwell<-cbind(emp_dwell[,1],state_column,emp_dwell[,2:4],true_cols[,1:2],emp_dwell[,5:ncol(emp_dwell)],true_cols[,3:4])
     colnames(emp_dwell)<-c("model","state","observations","dwell_time","estimates", "mean_emp_dwell_true","median_emp_dwell_true","mean_emp_dwell","median_emp_dwell", "sd_emp_dwell","ci_025","ci_975","ci_025_true","ci_975_true")
  ...
  saveRDS(list_post_est, "/gpfs/home1/adacko/medhmm_run/outputs/post_processed/list_post_est.rds")
   saveRDS(list_decod_acc, "/gpfs/home1/adacko/medhmm_run/outputs/post_processed/list_decod_acc.rds")
   saveRDS(list_switches_est, "/gpfs/home1/adacko/medhmm_run/outputs/post_processed/list_switches_est.rds")
   saveRDS(list_dwell_emp, "/gpfs/home1/adacko/medhmm_run/outputs/post_processed/list_dwell_emp.rds")
 }
