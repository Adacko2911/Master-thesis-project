...
#==================================================================================================================================================================================================
  if(m==3){
    ...
    sim_data<-readRDS(paste0("./data/three_state_design_2dep_final.rds"))
    ...
    if(m==4){
      #simulated data
      #In order to run the scripts without any issues using snellius we need to adjust the below data directory
      sim_data<-readRDS(paste0("./data/four_state_design_2dep_final.rds"))
      #---Emission distribution starting values
      emiss_distr <- matrix(c(10,20*1.2,
                              30, 60*1.2,
                              60,120*1.2,
      ...
      if(d==1){
        ...
        gamma   <- matrix(c(0.70, 0.1,0.1,0.1,
                            0.1,0.7,0.1,0.1,
                            0.1,0.1,0.7,0.1,
                            0.1,0.1,0.1,0.7), ncol = m, byrow = TRUE)
      }else if(d==3){
...
# Save output as a list of 100 repetitions of 36 scenarios:
#In order to run the scripts without any issues using snellius we need to adjust the below data directory
...
if(d==1){
  ...
  true_gamma_prob_bar<-as.vector(matrix(c(0.5, 0.5,
                                  0.5, 0.5), ncol = m, byrow = TRUE))
  true_gamma_prob_bar_no_diag<-as.vector(matrix(c(0, 1,
                                                  1, 0), ncol = m, byrow = TRUE))
}else if(d==2){
  true_dwell<-rep(exp_mean2,m)
  true_gamma_prob_bar<-as.vector(matrix(c(0.75, 0.25,
                                 0.25, 0.75), ncol = m, byrow = TRUE))
  true_gamma_prob_bar_no_diag<-as.vector(matrix(c(0, 1,
                                                  1, 0), ncol = m, byrow = TRUE))
}else if(d==3){
  true_dwell<-rep(exp_mean3,m)
  true_gamma_prob_bar<-as.vector(matrix(c(0.95, 0.05,
                                  0.05, 0.95), ncol = m, byrow = TRUE))
  true_gamma_prob_bar_no_diag<-as.vector(matrix(c(0, 1,
                                                  1, 0), ncol = m, byrow = TRUE))
  }else if(d==4){
  true_dwell<-rep(exp_mean4,m)
  true_gamma_prob_bar<-as.vector(matrix(c(0.99, 0.01,
                                  0.01, 0.99), ncol = m, byrow = TRUE))
  true_gamma_prob_bar_no_diag<-as.vector(matrix(c(0, 1,
                                                  1, 0), ncol = m, byrow = TRUE))
  }
